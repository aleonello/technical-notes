...
server = None
if options.usessl:
	server = smtplib.SMTP_SSL(serveraddr, options.serverport)
else:
	server = smtplib.SMTP(serveraddr, options.serverport)
...
* To call:
```sh
python3 smtptest.py -t -n <port_number> -u <user> -p <password> -v -d 1 <email_from> <email_to> <smtp_server_domain>
```
